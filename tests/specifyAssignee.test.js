...
const actionValue = {
  ...
  sheet_url: 'https://example.feishu.cn/sheets/token123',
  sheet_id: '288afd',
  row_index: 12,
  date_field_id: 'A',
...
class Store {
  constructor() { this.pending = new Map(); this.state = { roster: ['周杰', '张三'], off_duty: [] }; }
...
test('指定人员：创建单条需求表单并写回 AD 回扫人列', async () => {
  ...
  assert.match(JSON.stringify(client.calls[0].card), /assignee_name/);
...
test('指定人员：目标单元格已有其他负责人时拒绝覆盖', async () => {
  const store = new Store();
  ...
  assert.equal(store.pending.get('om_specify_form').completed_at, null);
});
