...
function legacySheetFields(sheetId, dateFieldId, dateFieldName, assigneeFieldId, assigneeFieldName) {
  ...
  return { dateFieldId, dateFieldName, assigneeFieldId, assigneeFieldName };
}
...
export function validateDispatchValue(value) {
  ...
  const sheetFields = legacySheetFields(
    sheetId,
    String(value.date_field_id ?? '').trim().toUpperCase(),
    String(value.date_field_name ?? '').trim(),
    String(value.assignee_field_id ?? '').trim().toUpperCase(),
    String(value.assignee_field_name ?? '').trim(),
  );
