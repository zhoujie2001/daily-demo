...
export const SPECIFY_ASSIGNEE_ACTION = 'bess_specify_assignee';
export const SCHEMA_VERSION = 1;
export const AD_SHEET_ID = '288afd';
export const AD_SPREADSHEET_URL = 'https://bytedance.larkoffice.com/sheets/QcX5sFcY0hw8w2tXxPbczs6wnog';
const AD_TARGET_CATEGORIES = new Set(['qianchuan_ad', 'ehc_emergency_ad']);
...
function normalizeSheetUrl({ sheetUrl, sheetId, targetCategory }) {
  const raw = String(sheetUrl ?? '').trim();
  if (sheetId === AD_SHEET_ID && AD_TARGET_CATEGORIES.has(targetCategory)) {
    return AD_SPREADSHEET_URL;
  }
  return raw;
}
...
export function validateDispatchValue(value) {
  ...
  const createdAt = String(value.created_at ?? '').trim() || String(value.created_at_raw ?? '').trim();
  const creator = String(value.creator ?? '').trim() || String(value.creator_name ?? '').trim();
...
export function validateSpecifyAssigneeValue(value) {
  if (!value || typeof value !== 'object' || value.action !== SPECIFY_ASSIGNEE_ACTION) {
    throw new DispatchValidationError('UNSUPPORTED_ACTION', '未识别的指定人员操作，已忽略');
  }
  return validateDispatchValue({ ...value, action: DISPATCH_ACTION });
}
...
export function validateAdjustStatusValue(value) {
  ...
  return {
    ...
    requestId: String(value.request_id || '').trim(),
