...
export function buildBatchDispatchCard(fieldsList, actionValue, { cardTitle, batchId } = {}) {
  const first = fieldsList[0];
  const elements = [
    { tag: 'markdown', content: `共 **${fieldsList.length}** 条 E 段需求，点击一次将按顺序处理全部需求。\n批次：${batchId || '-'}` },
  ...
  return {
    ...
    config: { update_multi: true },
    header: {
      title: { tag: 'plain_text', content: cardTitle || `【${first.businessType}】E 段自动派单` },
...
export function buildBatchStatusCard(fieldsList, {
  ...
  if (resumable) {
    elements.push({
      tag: 'markdown',
      content: '**处理中，可重试继续**：本轮已在安全截止时间前暂停，点击下方按钮可由同一批次继续未完成项。',
    });
  }
  elements.push({
    ...
    text: {
      tag: 'plain_text',
      content: resumable ? '▶️ 继续批量自动派单' : status === 'PROCESSING' ? '⏳ 批量派单处理中' : '✅ 批量派单已完成',
    },
...
export function buildBatchDispatchResultCard(fieldsList, {
  batchId, status, results = [], direction, dispatchedAt, cardTitle,
} = {}) {
  const resultMap = new Map(results.map((result) => [result.requestId, result]));
