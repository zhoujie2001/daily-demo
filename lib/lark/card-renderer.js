...
import { dispatchActionValue } from '../dispatch/ingest.js';
import { dispatchDirection } from '../dispatch/roster.js';
...
function dispatchedNote(requestId, dispatchedAt, assignmentMode = 'auto', assignee = '') {
  const detail = assignmentMode === 'specified'
    ? `已指定负责人${assignee ? ` ${assignee}` : ''}`
    : '派单话题已创建';
  return {
    tag: 'markdown',
    content: `${DISPATCH_DONE_PREFIX} ${requestId}：${detail}（${dispatchedAt}），按钮已锁定，请勿重复点击。`,
  };
}

function appendRequestMeta(details, fields) {
  if (fields.requestId) details.push(`- 需求 ID：${fields.requestId}`);
  if (fields.createdAt) details.push(`- 创建时间：${fields.createdAt}`);
  if (fields.creator) details.push(`- 创建人：${fields.creator}`);
}

function specifyAssigneeActionValue(fields) {
  return { ...dispatchActionValue(fields), action: 'bess_specify_assignee' };
}
...
export function buildInitialDispatchCard(fields, actionValue) {
  ...
  appendRequestMeta(details, fields);
  ...
  return {
    ...
    body: { elements: [
      ...
      {
        tag: 'button',
        element_id: `spec_${fields.requestId}`,
        text: { tag: 'plain_text', content: '指定人员' },
        type: 'default',
        behaviors: [{ type: 'callback', value: specifyAssigneeActionValue(fields) }],
      },
    ] },
  };
}
    ...
    ...fieldsList.flatMap((fields, index) => [
      ...(index > 0 ? [{ tag: 'hr' }] : []),
      {
        ...
        content: [
          ...
          `- 业务类型：${fields.businessType}`,
          '- 状态：READY',
          ...(fields.createdAt ? [`- 创建时间：${fields.createdAt}`] : []),
          ...(fields.creator ? [`- 创建人：${fields.creator}`] : []),
          ...(fields.rowIndex ? [`- 填入行：第 ${fields.rowIndex} 行`] : []),
        ].join('\n'),
      },
      {
        tag: 'button',
...
export function buildBatchDispatchResultCard(fieldsList, {
  batchId, status, results = [], roster = [], offDuty = [], direction, dispatchedAt, cardTitle, replacementNotice,
} = {}) {
  const resultMap = new Map(results.map((result) => [result.requestId, result]));
  const directionLabel = direction === 'forward' ? '正序（从上到下）' : '倒序（从下到上）';
  const businessType = String(fieldsList[0]?.businessType || '').trim() || '未标注';
  const isAd = businessType.toUpperCase() === 'AD';
  const fieldsByRequestId = new Map(fieldsList.map((fields) => [fields.requestId, fields]));
  const currentAssignees = new Map();
  results.forEach((result) => {
    if (result.status !== 'SUCCESS' || !result.assignee) return;
    const resultBusinessType = result.businessType
      || fieldsByRequestId.get(result.requestId)?.businessType
      || businessType;
    currentAssignees.set(dispatchDirection(resultBusinessType), result.assignee);
  });
  // Backward compatibility for persisted results created before businessType was
  ...
  if (currentAssignees.size === 0) {
    ...
    if (currentAssignee) currentAssignees.set(direction, currentAssignee);
  }
  const rosterNames = Array.isArray(roster) ? roster : [];
  const rosterContent = (names, listDirection) => names.map((name, index) => {
    const isOff = Array.isArray(offDuty) && offDuty.includes(name);
    const marker = name === currentAssignees.get(listDirection)
      ? ' **← 当前人员**' : isOff ? ' *(已离岗)*' : '';
    const nameText = isOff ? `~~${name}~~` : name;
    return `${index + 1}. ${nameText}${marker}`;
  }).join('\n') || '-';
  ...
  return {
    ...
    body: { elements: [
      ...
      ...(isAd ? [{
        tag: 'markdown',
        content: `🟣 **AD业态在班名单**\n${rosterContent(direction === 'forward' ? rosterNames : [...rosterNames].reverse(), direction)}`,
      }] : [{ tag: 'column_set', columns: [
        {
          tag: 'column', width: 'weighted', weight: 1,
          elements: [{ tag: 'markdown', content: `🟡 **千川正序（从上到下）**\n${rosterContent(rosterNames, 'forward')}` }],
        },
        {
          tag: 'column', width: 'weighted', weight: 1,
          elements: [{ tag: 'markdown', content: `🔵 **本地倒序（从下到上）**\n${rosterContent([...rosterNames].reverse(), 'reverse')}` }],
        },
      ] }]),
      { tag: 'hr' },
      {
        tag: 'button',
        text: { tag: 'plain_text', content: '⚙️ 调整人员状态' },
        type: 'default',
        value: { action: 'bess_adjust_status', batch_id: batchId, business_type: fieldsList[0]?.businessType },
      },
    ] },
  };
}
...
export function patchCardForDispatched(card, { requestId, threadMessageId, dispatchedAt, assignee, assignmentMode = 'auto' }) {
  ...
  if (targetTop) {
    walkElements([targetTop], (element) => {
      ...
      if (buttonMatchesRequest(element, requestId)) {
        ...
        element.disabled_tips = { tag: 'plain_text', content: assignmentMode === 'specified' ? '该需求已指定负责人' : '该需求已创建派单话题' };
        element.type = 'default';
        element.text = { tag: 'plain_text', content: assignmentMode === 'specified' ? '✅ 已指定' : DISPATCHED_BUTTON_TEXT };
        element.behaviors = [];
      }
    });
  }
  ...
  if (!noteAlreadyExists(cloned.body.elements, requestId)) {
    const note = dispatchedNote(requestId, dispatchedAt, assignmentMode, assignee);
    let inserted = false;
...
export function buildDispatchThreadText(fields, { dispatchedAt } = {}) {
  ...
  if (fields.createdAt) {
    lines.push(`需求创建时间：${fields.createdAt}`);
  }
  if (fields.creator) {
    lines.push(`需求创建人：${fields.creator}`);
  }
  if (fields.rowIndex) {
...
function rosterLines(roster, selected, offDuty = []) {
  return roster.map((name, index) => {
    const isOff = Array.isArray(offDuty) && offDuty.includes(name);
    const marker = name === selected ? ' **← 本次负责人**' : isOff ? ' *(已离岗)*' : '';
    const nameText = isOff ? `~~${name}~~` : name;
    return `${index + 1}. ${nameText}${marker}`;
  }).join('\n');
}

export function buildSpecifyAssigneeFormCard(fields, roster) {
  const options = roster.map((name) => ({ text: { tag: 'plain_text', content: name }, value: name }));
  return {
    schema: '2.0',
    config: { update_multi: true },
    header: { title: { tag: 'plain_text', content: '指定需求负责人' }, template: 'blue' },
    body: { elements: [
      { tag: 'markdown', content: `**需求 ${fields.requestId}｜${fields.requestName}**\n请从该业态今日在班人员名单中选择负责人。若目标单元格已有其他负责人，将拒绝覆盖。` },
      { tag: 'form', name: 'dispatch_specify_assignee_form', elements: [
        {
          tag: 'select_static', name: 'assignee_name', required: true, options,
          placeholder: { tag: 'plain_text', content: '请选择负责人' },
        },
        {
          tag: 'button', name: 'dispatch_specify_assignee_submit',
          text: { tag: 'plain_text', content: '确认指定' }, type: 'primary_filled', form_action_type: 'submit',
        },
      ] },
    ] },
  };
...
export function buildSpecifyAssigneeCompletedCard(fields, { assignee, completedAt }) {
  return {
    schema: '2.0',
    config: { update_multi: true },
    header: { title: { tag: 'plain_text', content: '✅ 已指定负责人' }, template: 'green' },
    body: { elements: [
      { tag: 'markdown', content: `**需求：** ${fields.requestId}｜${fields.requestName}\n**负责人：** ${assignee}\n**写回位置：** ${fields.assigneeFieldName || fields.assigneeFieldId}（第 ${fields.rowIndex} 行）\n**完成时间：** ${completedAt}` },
    ] },
  };
}
...
export function buildRosterSavedCard(fields) {
  const businessType = String(fields.businessType || '').trim() || '当前业态';
  return {
    schema: '2.0',
    config: { update_multi: true },
    header: { title: { tag: 'plain_text', content: `✅ ${businessType} 名单已保存` }, template: 'green' },
    body: { elements: [
      { tag: 'markdown', content: `**需求：** ${fields.requestName}\n今日${businessType}在班名单已保存。请在话题中的“指定人员”卡片继续选择负责人。` },
    ] },
  };
}

export function buildRosterCompletedCard(fields, { assignee, direction, dispatchedAt }) {
  const isAd = String(fields.businessType || '').trim().toUpperCase() === 'AD';
  const directionLabel = isAd
    ? 'AD业态在班名单'
    : direction === 'forward' ? '千川正序（从上到下）' : '倒序（从下到上）';
  return {
    schema: '2.0',
    config: { update_multi: true },
...
export function buildDispatchFailureCard(errorMessage = '派单未完成，请稍后重试') {
  return {
    schema: '2.0',
    config: { update_multi: true },
    header: { title: { tag: 'plain_text', content: '派单失败' }, template: 'red' },
    body: { elements: [
      { tag: 'markdown', content: `**${errorMessage}**\n原需求卡保持不变，修复后可重新点击派单按钮。` },
    ] },
  };
}

export function buildDispatchResultCard(fields, { assignee, direction, roster, offDuty = [], dispatchedAt }) {
  const isAd = String(fields.businessType || '').trim().toUpperCase() === 'AD';
  const forward = rosterLines(roster, direction === 'forward' ? assignee : null, offDuty);
  const reverse = rosterLines([...roster].reverse(), direction === 'reverse' ? assignee : null, offDuty);
  const directionLabel = direction === 'forward' ? '正序（从上到下）' : '倒序（从下到上）';
  const rosterElements = isAd
    ? [{ tag: 'markdown', content: `🟣 **AD业态在班名单**\n${direction === 'forward' ? forward : reverse}` }]
    : [{ tag: 'column_set', columns: [
  ...
  return {
    ...
    body: { elements: [
      { tag: 'markdown', content: `**需求：** ${fields.requestName}\n**负责人：** ${assignee}\n**方向：** ${isAd ? 'AD业态在班名单' : directionLabel}\n**时间：** ${dispatchedAt}` },
      { tag: 'hr' },
      ...rosterElements,
      { tag: 'hr' },
      {
        ...
        text: { tag: 'plain_text', content: '⚙️ 调整人员状态' },
        type: 'default',
        value: { action: 'bess_adjust_status', request_id: fields.requestId, business_type: fields.businessType },
      },
    ] },
  };
}

export function buildAdjustStatusFormCard({ dayKey, businessType, roster, offDuty, version }) {
  const options = roster.map((name) => ({
    text: { tag: 'plain_text', content: offDuty.includes(name) ? `${name} (已离岗)` : name },
    value: name,
  }));
  return {
    schema: '2.0',
    config: { update_multi: true },
    header: { title: { tag: 'plain_text', content: '今日人员状态调整' }, template: 'yellow' },
    body: { elements: [
      { tag: 'markdown', content: `**业务类型：** ${businessType}\n**日期：** ${dayKey}\n\n当前离岗：${offDuty.join('、') || '无'}` },
      {
        tag: 'form', name: 'adjust_status_form', elements: [
          {
            tag: 'select_static', name: 'target_name', required: true,
            placeholder: { tag: 'plain_text', content: '请选择要调整的人员' },
            options,
          },
          {
            tag: 'select_static', name: 'op_type', required: true,
            placeholder: { tag: 'plain_text', content: '请选择临时离岗或恢复接单' },
            options: [
              { text: { tag: 'plain_text', content: '临时离岗' }, value: 'leave' },
              { text: { tag: 'plain_text', content: '恢复接单' }, value: 'resume' },
            ],
          },
          {
            tag: 'input', name: 'reason', input_type: 'text', required: true,
            placeholder: { tag: 'plain_text', content: '例如：临时开会、紧急支持等' },
            max_length: 100,
          },
          {
            tag: 'button', name: 'adjust_status_submit',
            text: { tag: 'plain_text', content: '提交调整' },
            type: 'primary_filled', form_action_type: 'submit',
            value: { action: 'bess_adjust_status_submit', day_key: dayKey, business_type: businessType, expected_version: version },
          },
        ],
      },
    ] },
...
export function buildAdjustStatusSuccessCard({ targetName, opType, reason }) {
  const statusLabel = opType === 'leave' ? '临时离岗' : '恢复接单';
  return {
    schema: '2.0',
    config: { update_multi: true },
    header: { title: { tag: 'plain_text', content: '人员状态调整成功' }, template: 'green' },
    body: { elements: [
      { tag: 'markdown', content: `✅ **${targetName}** 已更新为「${statusLabel}」\n\n调整原因：${reason || '未填写原因'}\n后续尚未处理的需求将使用最新人员状态。` },
    ] },
  };
}
