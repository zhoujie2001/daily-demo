    ...
    body: { elements: [
      { tag: 'markdown', content: `**业务类型：** ${businessType}\n**日期：** ${dayKey}\n\n当前离岗：${offDuty.join('、') || '无'}` },
      {
        tag: 'form', name: 'adjust_status_form', elements: [
          {
            tag: 'select_static', name: 'target_name', label: { tag: 'plain_text', content: '选择人员' },
            placeholder: { tag: 'plain_text', content: '请选择要调整的人员' },
            options,
          },
          {
            tag: 'radio', name: 'op_type', label: { tag: 'plain_text', content: '操作类型' },
            options: [
          ...
          {
            tag: 'input', name: 'reason', label: { tag: 'plain_text', content: '调整原因' },
            placeholder: { tag: 'plain_text', content: '例如：临时开会、紧急支持等' },
            max_length: 100,
          },
          {
            tag: 'button', text: { tag: 'plain_text', content: '提交调整' },
