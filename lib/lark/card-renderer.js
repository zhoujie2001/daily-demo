...
export function buildBatchDispatchCard(fieldsList, actionValues, { cardTitle } = {}) {
  const first = fieldsList[0];
  const elements = [
    { tag: 'markdown', content: `共 **${fieldsList.length}** 条 E 段需求，请逐条点击对应按钮派单。` },
  ];
  fieldsList.forEach((fields, index) => {
    ...
    elements.push({
      tag: 'column_set',
      columns: [
        { tag: 'column', width: 'weighted', weight: 4, elements: [{ tag: 'markdown', content: details.join('\n') }] },
        { tag: 'column', width: 'weighted', weight: 1, vertical_align: 'center', elements: [{
          tag: 'button',
          element_id: `dsp_${fields.requestId}`,
          text: { tag: 'plain_text', content: '🎯 自动派单' },
          type: 'primary_filled',
          behaviors: [{ type: 'callback', value: actionValues[index] }],
        }] },
      ],
    });
  });
