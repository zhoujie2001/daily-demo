...
export class LarkClient {
  ...
  async readSheetDispatchRows({
    sheetUrl, sheetId, dateFieldId, dateFieldName, assigneeFieldId, assigneeFieldName, selectLatest,
  }) {
    const token = await this.getTenantAccessToken();
    const spreadsheetToken = await this.resolveSpreadsheetToken(sheetUrl, token);
    const [dateColumn, assigneeColumn, metadata] = await Promise.all([
    ...
    for (let startRow = 1; startRow <= rowCount; startRow += SHEET_SCAN_CHUNK_ROWS) {
      ...
      const ranges = [dateColumn, assigneeColumn].map((column) => `${sheetId}!${column}${startRow}:${column}${chunkEnd}`);
      ...
      const length = Math.max(values[0].length, values[1].length);
      const chunkRows = [];
      for (let index = 0; index < length; index += 1) {
        chunkRows.push([values[0][index]?.[0], values[1][index]?.[0]]);
      }
