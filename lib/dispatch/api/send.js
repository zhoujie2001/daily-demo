    ...
    try {
      ...
      if (Array.isArray(body?.items)) {
        ...
        for (let attempt = 1; attempt <= 3; attempt += 1) {
          try {
            claim = await store.claimIngestBatch({
            ...
            claimError = error;
            const transient = ['DISPATCH_DB_ERROR', 'DISPATCH_DB_TIMEOUT'].includes(error?.code);
        ...
        const backgroundSend = (async () => {
          ...
          if (stateError) {
            // The Lark UUID is deterministic. Keep SENDING until its lease expires;
            // a later recovery can safely resend the same logical message.
