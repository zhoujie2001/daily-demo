...
export function resolveSyncWaitMs(req) {
  ...
  try {
    ...
    // malformed URL — treat as no wait
  }
  if (String(req.headers?.['x-bess-wait'] || '').trim().toLowerCase() === 'sync') explicit = true;
...
export function createDispatchSendHandler({
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      const syncWaitMs = resolveSyncWaitMs(req);
      if (syncWaitMs > 0) {
