    ...
    try {
      ...
      if (Array.isArray(body?.items)) {
        ...
        const claim = await store.claimIngestBatch({
          chatId, batchId, fingerprint, attemptToken,
        ...
        const backgroundSend = (async () => {
          ...
          try {
            ...
            try {
              ...
              log('error', 'batch_failure_state_write_failed', {
                source, batch_id: batchId, chat_id: chatId,
                error_code: stateError?.code || 'INGEST_STATE_WRITE_FAILED',
              });
            }
            log('error', 'batch_send_failed', {
              source, batch_id: batchId, chat_id: chatId,
          ...
          try {
            ...
            for (let attempt = 1; attempt <= 3; attempt += 1) {
              try {
                await store.completeIngestBatch({ chatId, batchId, fingerprint, attemptToken, requestIds, messageId: message.message_id, completedAt: now() });
                stateError = null;
                break;
              } catch (error) {
                stateError = error;
            ...
            // The remote side effect already happened. Leave the durable state in
            // SENDING so callers fail closed and retry status instead of re-sending.
            log('error', 'batch_delivery_state_write_failed', {
              source, batch_id: batchId, chat_id: chatId, message_id: message.message_id,
              error_code: error?.code || 'INGEST_STATE_WRITE_FAILED',
            });
          }
        })();
        defer(backgroundSend);
      ...
      log('info', 'sent', {
        ...
        request_id: fields.requestId, chat_id: chatId, message_id: message.message_id,
      });
