...
async function enrichRejectReasonsWithinDeadline({
  ...
  if (result.timedOut) {
    log('error', 'local_promo_reject_reason_lookup_timeout', {
      source, chat_id: chatId, timeout_ms: timeoutMs,
    });
    throw Object.assign(new Error('Reject reason lookup timed out'), {
      code: 'REJECT_REASON_LOOKUP_TIMEOUT', status: 503,
    });
  }
  for (let index = 0; index < items.length; index += 1) {
    if (!String(items[index]?.reject_reason ?? items[index]?.rejectReason ?? '').trim()
        && result.lookupItems[index]?.reject_reason !== undefined) {
