...
export function createDispatchStatusHandler({
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      let store;
      if (status) {
        res.setHeader('X-Bess-Status-Source', 'runtime-cache');
      } else {
        store = storeFactory();
        const databaseStartedAt = Date.now();
        status = await store.getIngestBatchStatus({ chatId, batchId });
        timing.measure('database', databaseStartedAt);
        res.setHeader('X-Bess-Status-Source', 'supabase');
        await writeDispatchStatusCache(statusCache, {
          chatId, batchId,
          value: status.found
            ? status
            : queuedDispatchStatus({ operationId: dispatchOperationId(chatId, batchId) }),
        });
      }
      if (!status.found) {
        // Queue acceptance is durable before the Supabase ledger is materialized.
        // A missing row is therefore a transient queued state, never permission
        // to send the same card through an unrelated fallback path.
        timing.flush();
        return res.status(200).json({
          ok: true, found: false, status: 'QUEUED', transient: true,
          operation_id: dispatchOperationId(chatId, batchId), retry_after_ms: 2_000,
        });
      }
