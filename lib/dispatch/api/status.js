...
export function createDispatchStatusHandler({
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      // The synchronous sender stores SENDING without an outbox card payload.
      // Once its lease expires, exposing it as retryable FAILED lets the caller
      // resubmit the exact same batch. /send then resumes the CAS lease and uses
      // the same stable Lark UUID, instead of handing an incomplete row to the
      // outbox worker or leaving the caller blocked forever.
