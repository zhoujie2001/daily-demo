...
export function createDispatchStatusHandler({
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      if (status.found && ['QUEUED', 'RETRY', 'PROCESSING'].includes(status.status)) {
        // Only an atomic, bounded database nudge: no Lark/Sheet call is allowed here.
        // Recovery is best-effort; a nudge timeout must not hide a status we already read.
        try { await store.nudgeDispatchOutbox({ chatId, batchId }); } catch { /* cron remains authoritative */ }
      }
      const publicStatus = ['QUEUED', 'RETRY', 'PROCESSING'].includes(status.status) ? 'SENDING' : status.status;
      return res.status(200).json({ ok: true, ...status, ...(publicStatus ? { status: publicStatus } : {}) });
    } catch (error) {
