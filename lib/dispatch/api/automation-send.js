...
async function enrichRejectReasonsWithinDeadline({
  ...
  try {
    ...
    log('warn', 'enrichment_timeout', { source, chat_id: chatId, timeout_ms: timeoutMs });
    return [];
  } finally {
...
export function createAutomationSendHandler({
  ...
  return async function automationSendHandler(req, res) {
    ...
    if (!secret) {
      return res.status(503).json({ ok: false, error_code: 'AUTOMATION_DISPATCH_NOT_CONFIGURED' });
    }
    ...
    if (typeof body.dispatch_payload === 'string') {
      try { payload = JSON.parse(body.dispatch_payload); } catch {
