...
// validate -> create thread via reply -> build and actively update original
// card -> include the same card in the callback response as an immediate and
...
export async function handleDispatchEvent(body, {
  ...
  try {
    ...
    let cardUpdateApiSucceeded = false;
    try {
      await client.updateMessageCard(event.messageId, updatedCard);
      cardUpdateApiSucceeded = true;
      log('card_update_api_succeeded', {
        ...logBase,
        card_build_mode: cardBuildMode,
        duration_ms: duration(),
      });
    } catch (error) {
      // The thread already exists. Keep the dispatch successful and let the
      // callback response.card perform the immediate/compatibility update.
      log('card_update_api_failed', {
        ...logBase,
        error_code: error instanceof LarkApiError ? error.code : 'CARD_UPDATE_API_FAILED',
        card_build_mode: cardBuildMode,
        duration_ms: duration(),
      });
    }
    ...
    log('succeeded', {
      ...logBase,
      card_update_api_succeeded: cardUpdateApiSucceeded,
      card_response_included: true,
      card_update_mode: cardUpdateApiSucceeded ? 'api_with_response_fallback' : 'response_fallback',
      card_build_mode: cardBuildMode,
      duration_ms: duration(),
    });
    ...
    log('failed', { ...logBase, error_code: code, duration_ms: duration() });
    return {
      httpStatus: 200,
