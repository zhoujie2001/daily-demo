...
// validate -> create thread via reply -> patch original card -> respond.
//
...
export async function handleDispatchEvent(body, {
  ...
  try {
    ...
    if (originalMessage && originalMessage.msg_type === 'interactive' && originalMessage.body?.content) {
      ...
      if (originalCard && originalCard.schema === '2.0') {
        ...
        if (result.patched) {
          responseBody.card = { type: 'raw', data: result.card };
          cardUpdated = true;
        } else {
          log('card_patch_skipped', { ...logBase, error_code: 'DISPATCH_BUTTON_NOT_FOUND', duration_ms: duration() });
        }
      }
    }

    if (!cardUpdated) {
      responseBody = toast('warning', '派单话题已创建；如按钮未变灰，请重新打开卡片查看');
    }

    completed.add(fields.requestId);
    log('succeeded', { ...logBase, card_updated: cardUpdated, duration_ms: duration() });
