...
async function handleBatchDispatch({ event, batch, store, client, now, log, logBase, config, roster, formMessageId }) {
  ...
  const afterResponse = async () => {
    ...
    if (formMessageId && formMessageId !== targetMessageId) {
      await client.updateMessageCard(formMessageId, buildRosterCompletedCard(batch.items[0], {
        assignee: latestResults.find((r) => r.status === 'SUCCESS')?.assignee || '批量处理完成',
        direction: dispatchDirection(batch.items[0].businessType),
        dispatchedAt,
      })).catch((error) => logUiFailure('update_roster_form_card', error));
      await store.markPendingCompleted(formMessageId, now()).catch(() => {});
    }
