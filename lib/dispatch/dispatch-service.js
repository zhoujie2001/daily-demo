...
import { inspectLatestDailyAnchor } from './sheet-anchor.js';
import {
...
async function readTargetAndAnchor({ client, fields, dayKey, roster, store }) {
  ...
  let anchorResult = { anchor: null, hasRelevantAssignment: false, invalidAssignee: null };
  await client.readSheetDispatchRows({
    ...
    selectLatest: (rows, startRow) => {
      ...
      const limit = Math.min(rows.length, Math.max(0, relativeIndex));
      if (limit > 0) {
        const candidate = inspectLatestDailyAnchor(rows.slice(0, limit), {
          targetDay: dayKey, roster, projectValue: fields.projectValue,
        });
        if (candidate.anchor || candidate.hasRelevantAssignment) anchorResult = candidate;
      }
...
async function p1Dispatch({ event, fields, store, client, now, log, logBase, config }) {
  ...
  const {
    valueAtTargetRow, existingAssignment, anchor, hasRelevantAssignment, invalidAssignee,
  ...
  if (!valueAtTargetRow && !existingAssignment && hasRelevantAssignment && !anchor) {
    throw new DispatchValidationError(
      'INVALID_SHEET_ANCHOR',
      `检测到不在今日名单中的负责人${invalidAssignee ? `：${invalidAssignee}` : ''}，请校准表格后重试`,
    );
  }
  ...
  if (!assigned.replayed && anchor && String(assigned.assignee).trim() === String(anchor).trim()) {
...
async function handleBatchDispatch({ event, batch, store, client, now, log, logBase, config, roster, formMessageId }) {
  ...
  const afterResponse = async () => {
    ...
    const threadDone = claim.thread_reply_done === true || await recordFinalization({
      ...
      operation: () => client.replyInteractiveCard({
