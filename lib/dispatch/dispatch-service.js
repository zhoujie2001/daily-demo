...
async function handleAdjustStatusSubmit({ event, adjustSubmit, store, client, now, log, logBase }) {
  ...
  const afterResponse = async () => {
    try {
      const original = await client.getMessage(event.messageId);
      if (original?.msg_type === 'interactive' && original.body?.content) {
        // 如果是结果卡，尝试刷新它
        // 注意：简单起见，此处仅通过 Toast 提示刷新成功，
        // 实际上完整的卡片刷新需要 request_context，
        // 建议用户刷新话题卡或在下次派单时自动看到更新。
      }
    } catch { /* ignore */ }
  };
