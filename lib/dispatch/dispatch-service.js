  ...
  await client.readSheetDispatchRows({
    ...
    selectLatest: (rows, startRow) => {
      ...
      if (limit > 0) {
        ...
        // Chunks arrive in sheet order. Any relevant assignment in a newer
        // chunk, including an invalid one, supersedes and invalidates old state.
        if (candidate.hasRelevantAssignment) anchorResult = candidate;
      }
...
async function p1Dispatch({ event, fields, store, client, now, log, logBase, config }) {
  ...
  const {
    ...
    hasRelevantAssignment, invalidAssignee,
...
export async function handleDispatchEvent(body, {
  ...
  if (persistentStore) {
    try {
      ...
      const failureBody = toast('error', validation ? (error.userMessage || error.message) : '派单失败，请稍后重试或联系值班同学');
