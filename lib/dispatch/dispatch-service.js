...
async function readTargetAndAnchor({ client, fields, dayKey, roster, direction, store }) {
  ...
  const anchorAssignee = await client.readSheetDispatchRows({
    ...
    selectLatest: (rows, startRow) => {
      ...
      const limit = Math.min(rows.length, relativeIndex);
      return limit > 0 ? latestDailyAnchor(rows.slice(0, limit), {
  ...
  if (anchorAssignee && !existingAssignment) {
    const anchorIndex = roster.findIndex((name) => String(name).trim() === anchorAssignee);
    if (anchorIndex >= 0) {
      const cursor = direction === 'forward' ? anchorIndex + 1 : roster.length - anchorIndex;
      await store.calibrateCursor({ dayKey, direction, cursor });
    }
  }
  return { valueAtTargetRow, existingAssignment };
}
...
async function p1Dispatch({ event, fields, store, client, now, log, logBase, config }) {
  ...
  const { valueAtTargetRow, existingAssignment } = await readTargetAndAnchor({
    client, fields: context, dayKey, roster: effectiveRoster, direction, store,
  });
...
async function dispatchBatchItem({ fields, store, client, current, roster }) {
  ...
  const { valueAtTargetRow, existingAssignment } = await readTargetAndAnchor({
    client, fields, dayKey, roster: effectiveRoster, direction, store,
  });
