...
async function dispatchBatchItem({ fields, store, client, current }) {
  validateSheetTarget(fields);
  const dayKey = shanghaiDay(current);
  const state = await store.getDailyState(dayKey, current);
  if (!state || !Array.isArray(state.roster) || state.roster.length === 0) {
  ...
  const assigned = await store.assign({
    dayKey, requestId: fields.requestId, direction, roster: null,
...
async function handleBatchDispatch({ event, batch, store, client, now, log, logBase, config }) {
  ...
  const afterResponse = async () => {
    ...
    for (const fields of batch.items) {
      ...
      let result;
      try {
        const assigned = await dispatchBatchItem({ fields, store, client, current: now() });
        result = {
