...
async function handleBatchDispatch({ event, batch, store, client, now, log, logBase, config, roster, formMessageId }) {
  ...
  const afterResponse = async () => {
    ...
    // Snapshot every still-pending target before assigning anything. A manual
    // value is a fixed point for the whole batch; the latest row wins (the same
    // deterministic rule used by sheet anchoring). Calibrate exactly once, then
    // let automatic rows advance one continuous persistent cursor. In particular,
    // crossing a prefilled row must not rewind that cursor.
    const effectiveRoster = state?.roster || roster;
    const batchReads = new Map();
    const batchReadErrors = new Map();
    let batchAnchor = null;
    let batchAnchorRequestId = '';
    let batchAnchorRow = -1;
    ...
    for (const fields of batch.items) {
      ...
      try {
        ...
        if (!fallbackAnchor && read.anchor && effectiveRoster.includes(read.anchor)) {
          fallbackAnchor = read.anchor;
          batchAnchorRequestId = fields.requestId;
        }
        if (read.valueAtTargetRow && effectiveRoster.includes(read.valueAtTargetRow)
            && !read.existingAssignment && fields.rowIndex >= batchAnchorRow) {
          batchAnchor = read.valueAtTargetRow;
          batchAnchorRequestId = fields.requestId;
          batchAnchorRow = fields.rowIndex;
        }
      } catch (error) {
        batchReadErrors.set(fields.requestId, error);
      }
    }
    batchAnchor ||= fallbackAnchor;
    if (batchAnchor) {
      try {
        await calibrateFilledTarget({ store, dayKey, scope, assignee: batchAnchor, roster: effectiveRoster });
      } catch (error) {
        batchReadErrors.set(batchAnchorRequestId, error);
      }
    }
    ...
    for (const fields of batch.items) {
      ...
      try {
        ...
        const assigned = await dispatchBatchItem({
          ...
          batchRead: batchReads.get(fields.requestId),
        });
        result = {
