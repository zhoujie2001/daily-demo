...
const INGEST_STALE_MS = 2 * 60 * 1000;
...
export function createSupabaseDispatchStore({
  ...
  async function getIngestBatchStatus({ chatId, batchId }) {
    ...
    const sendingStartedAt = Date.parse(context.sendingStartedAt || row.created_at || row.updated_at || '');
    const retryable = status === 'FAILED' || (
      ['SENDING', 'RECOVERING'].includes(status)
      && Number.isFinite(sendingStartedAt)
      && Date.now() - sendingStartedAt >= INGEST_STALE_MS
    );
    return {
      found: true,
      batch_id: String(context.batchId || batchId),
      chat_id: String(row.chat_id || chatId),
  ...
  return {
    ...
    async claimIngestBatch({ chatId, batchId, fingerprint, attemptToken, requestIds, expiresAt, now = new Date() }) {
      const key = ingestStorageKey(chatId, batchId);
      const context = {
        kind: 'dispatch_ingest', batchId, fingerprint, attemptToken, requestIds, status: 'SENDING',
      ...
      const sendingStartedAt = Date.parse(existing.sendingStartedAt || row?.created_at || row?.updated_at || '');
      const staleInFlight = ['SENDING', 'RECOVERING'].includes(existing.status)
        && Number.isFinite(sendingStartedAt)
        && now.getTime() - sendingStartedAt >= INGEST_STALE_MS;
      if (existing.status === 'FAILED' || staleInFlight) {
        const expectedStatus = existing.status;
        const oldAttemptFilter = existing.attemptToken
          ? `&request_context-%3E%3EattemptToken=eq.${encodeURIComponent(existing.attemptToken)}`
          : '';
        const rows = await request(`bess_dispatch_pending_forms?form_message_id=eq.${encodeURIComponent(key.formMessageId)}&request_context-%3E%3Efingerprint=eq.${encodeURIComponent(fingerprint)}&request_context-%3E%3Estatus=eq.${encodeURIComponent(expectedStatus)}${oldAttemptFilter}`, {
          method: 'PATCH',
          body: {
            request_context: {
              kind: 'dispatch_ingest', batchId, fingerprint, attemptToken, requestIds, status: 'RECOVERING',
              sendingStartedAt: now.toISOString(),
            },
            completed_at: null,
          },
        ...
        return rows?.[0] ? { outcome: 'CLAIMED' } : { outcome: 'IN_FLIGHT', message_id: '' };
      }
      return {
    ...
    async completeIngestBatch({ chatId, batchId, fingerprint, attemptToken, requestIds = [], messageId, completedAt = new Date() }) {
      const key = ingestStorageKey(chatId, batchId);
      const rows = await request(`bess_dispatch_pending_forms?form_message_id=eq.${encodeURIComponent(key.formMessageId)}&request_context-%3E%3Efingerprint=eq.${encodeURIComponent(fingerprint)}&request_context-%3E%3EattemptToken=eq.${encodeURIComponent(attemptToken)}`, {
        method: 'PATCH',
        body: {
          request_context: { kind: 'dispatch_ingest', batchId, fingerprint, attemptToken, requestIds, status: 'SENT', messageId },
    ...
    async failIngestBatch({ chatId, batchId, fingerprint, attemptToken, errorCode, failedAt = new Date() }) {
      const key = ingestStorageKey(chatId, batchId);
      const row = await getBatchRow(key.formMessageId);
      const existing = parseBatchContext(row);
      if (
        !existing
        || existing.kind !== 'dispatch_ingest'
        || existing.fingerprint !== fingerprint
        || existing.attemptToken !== attemptToken
        || !['SENDING', 'RECOVERING'].includes(existing.status)
      ) {
        throw new DispatchStoreError('INGEST_CLAIM_LOST', '发送幂等状态已失效', 409);
      }
      const rows = await request(`bess_dispatch_pending_forms?form_message_id=eq.${encodeURIComponent(key.formMessageId)}&request_context-%3E%3Efingerprint=eq.${encodeURIComponent(fingerprint)}&request_context-%3E%3EattemptToken=eq.${encodeURIComponent(attemptToken)}&request_context-%3E%3Estatus=eq.${encodeURIComponent(existing.status)}`, {
        method: 'PATCH',
        body: {
          request_context: {
            ...existing,
            status: 'FAILED',
            errorCode: String(errorCode || 'INGEST_FAILED').slice(0, 100),
