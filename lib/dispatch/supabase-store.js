...
export function createSupabaseDispatchStore({
  ...
  statusTotalTimeoutMs = 1350,
  statusRetryDelayMs = 50,
  ...
  function isRetryableStatusRead(error) {
    if (!(error instanceof DispatchStoreError)) return false;
    if (error.code === 'DISPATCH_DB_TIMEOUT') return true;
    if (error.code !== 'DISPATCH_DB_ERROR') return false;
    if (!error.httpStatus) return true;
    return [408, 425, 429, 500, 502, 503, 504, 520, 522, 524].includes(error.httpStatus);
  }

  async function readIngestBatchStatus(path) {
    const startedAt = Date.now();
    let lastError;
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      const remainingMs = statusTotalTimeoutMs - (Date.now() - startedAt);
      if (remainingMs <= 0) break;
      try {
        return await request(path, {
          requestTimeoutMs: Math.min(statusTimeoutMs, remainingMs),
        });
      } catch (error) {
        lastError = error;
        if (attempt >= 2 || !isRetryableStatusRead(error)) throw error;
        const delayMs = Math.max(0, Math.min(statusRetryDelayMs, statusTotalTimeoutMs - (Date.now() - startedAt)));
        logDbRequest('warn', {
          outcome: 'retry_scheduled', operation: 'GET bess_dispatch_pending_forms',
          attempt: attempt + 1, delay_ms: delayMs,
          previous_error_code: clean(error.code), http_status: error.httpStatus || null,
        });
        if (delayMs > 0) await new Promise((resolve) => setTimeout(resolve, delayMs));
      }
    }
    throw lastError || new DispatchStoreError('DISPATCH_DB_TIMEOUT', '派单状态查询超过总时限', 503);
  }

  function ingestStatusFromRow(row) {
  ...
  async function getIngestBatchStatus({ chatId, batchId }) {
    ...
    const rows = await readIngestBatchStatus(
    ...
    return ingestStatusFromRow(rows?.[0] || null);
  }

  async function getIngestBatchStatuses(items) {
    const targets = items.map(({ chatId, batchId }) => ({
      chatId,
      batchId,
      formMessageId: ingestStorageKey(chatId, batchId).formMessageId,
    }));
    if (targets.length === 0) return [];
    const ids = targets.map(({ formMessageId }) => formMessageId).join(',');
    const rows = await readIngestBatchStatus(
      `bess_dispatch_pending_forms?form_message_id=in.(${ids})&select=form_message_id,request_context&limit=${targets.length}`,
    );
    const byId = new Map((rows || []).map((row) => [String(row.form_message_id || ''), row]));
    return targets.map(({ chatId, batchId, formMessageId }) => ({
      chat_id: chatId,
      batch_id: batchId,
      ...ingestStatusFromRow(byId.get(formMessageId)),
    }));
  }
  ...
  return {
    ...
    async claimIngestBatch({ chatId, batchId, fingerprint, requestIds, now = new Date(), expiresAt }) {
      ...
      try {
        const payload = await request('rpc/bess_claim_ingest', {
          ...
          requestTimeoutMs: Math.min(2500, remaining()),
        });
        const result = Array.isArray(payload) ? payload[0] : payload;
    ...
    async getDailyState(day, scope = 'default', now = new Date()) {
      const rows = await request(`bess_dispatch_daily_state?day_key=eq.${encodeURIComponent(day)}&scope=eq.${encodeURIComponent(scope)}&expires_at=gt.${encodeURIComponent(now.toISOString())}&select=*&limit=1`);
      return rows?.[0] || null;
    },
    ...
    async markPendingCompleted(formMessageId, completedAt = new Date(), requestContext = undefined) {
      const body = { completed_at: completedAt.toISOString() };
      if (requestContext !== undefined) body.request_context = requestContext;
    ...
    async getAssignment(dayKey, requestId, scope = 'default') {
      const rows = await request(`bess_dispatch_assignments?day_key=eq.${encodeURIComponent(dayKey)}&scope=eq.${encodeURIComponent(scope)}&request_id=eq.${encodeURIComponent(requestId)}&select=*&limit=1`);
      return rows?.[0] || null;
    },
    async getDailyAssignments(dayKey, scope = 'default') {
      ...
      return await request(`bess_dispatch_assignments?day_key=eq.${encodeURIComponent(dayKey)}&scope=eq.${encodeURIComponent(scope)}&select=*&order=created_at.desc,id.desc`) || [];
    },
    async calibrateCursor({ dayKey, scope = 'default', assignee, roster }) {
      ...
      try {
        const rows = await request('rpc/bess_calibrate_cursor', {
          method: 'POST',
          body: { p_day_key: dayKey, p_scope: scope, p_assignee: normalizedAssignee, p_roster: normalizedRoster },
        });
      ...
      const currentRows = await request(`bess_dispatch_daily_state?day_key=eq.${encodeURIComponent(dayKey)}&scope=eq.${encodeURIComponent(scope)}&select=*&limit=1`);
      ...
      const rows = await request(`bess_dispatch_daily_state?day_key=eq.${encodeURIComponent(dayKey)}&scope=eq.${encodeURIComponent(scope)}&forward_cursor=eq.${encodeURIComponent(current.forward_cursor)}&reverse_cursor=eq.${encodeURIComponent(current.reverse_cursor)}&roster=eq.${encodeURIComponent(JSON.stringify(normalizedRoster))}`, {
        method: 'PATCH',
        body: {
    ...
    async updateRosterStatus({ dayKey, scope = 'default', offDuty, expectedVersion }) {
    ...
    async initializeRoster({ dayKey, scope = 'default', roster, expiresAt, current = new Date() }) {
      if (!dayKey || !Array.isArray(roster) || roster.length === 0 || !expiresAt) {
        throw new DispatchStoreError('INVALID_ROSTER_INITIALIZATION', '初始化当天派单名单参数无效', 400);
      }
      await request('bess_dispatch_daily_state?on_conflict=day_key,scope', {
        method: 'POST',
        body: {
          day_key: dayKey,
          scope,
      ...
      const state = await this.getDailyState(dayKey, scope, current);
      if (!state) throw new DispatchStoreError('DAILY_STATE_NOT_FOUND', '当天派单名单初始化失败');
      return state;
    },
    async assignSpecific({ dayKey, scope = 'default', requestId, assignee, context }) {
      const rows = await request('rpc/bess_assign_specific', {
        method: 'POST',
        body: {
          p_day_key: dayKey,
          p_scope: scope,
      ...
      if (!result?.assignee) throw new DispatchStoreError('SPECIFIC_ASSIGNMENT_FAILED', '指定负责人未返回有效结果');
      return result;
    },
    async assign({ dayKey, scope = 'default', requestId, direction, roster, expiresAt, context }) {
      const rows = await request('rpc/bess_assign_next', {
        method: 'POST',
        body: {
          p_day_key: dayKey, p_scope: scope, p_request_id: requestId, p_direction: direction,
