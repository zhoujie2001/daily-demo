...
export function createSupabaseDispatchStore({
  ...
  async function claimDispatchOutboxLegacy({ workerToken, limit, leaseSeconds, maxAttempts, now }) {
    const rows = await request(
      'bess_dispatch_pending_forms?request_context-%3E%3Ekind=eq.dispatch_ingest&completed_at=is.null&select=*&order=created_at.asc&limit=100',
    ) || [];
    const claimed = [];
