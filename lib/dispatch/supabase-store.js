...
export function createSupabaseDispatchStore({
    ...
    try {
      ...
      if (!response.ok) {
        ...
        logDbRequest('error', {
          ...
          timeout_ms: timeoutMs, http_status: response.status, request_id: requestId,
        });
        throw error;
      }
      ...
      logDbRequest('error', {
        ...
        duration_ms: durationMs, timeout_ms: timeoutMs, http_status: null,
  ...
  async function patchIngestContext({ formMessageId, expectedLeaseExpiresAt, context, completedAt }) {
    // Always narrow by the deterministic primary key first. The extra JSONB predicates
    // are compare-and-swap guards for the single matching row, not broad lookup filters.
    const leaseFilter = expectedLeaseExpiresAt
      ? `eq.${encodeURIComponent(expectedLeaseExpiresAt)}`
      : 'is.null';
    const rows = await request(`bess_dispatch_pending_forms?form_message_id=eq.${encodeURIComponent(formMessageId)}&request_context-%3E%3Estatus=eq.SENDING&request_context-%3E%3EleaseExpiresAt=${leaseFilter}`, {
      method: 'PATCH',
      body: {
        request_context: context,
        ...(completedAt === undefined ? {} : { completed_at: completedAt }),
  ...
  async function getIngestBatchStatus({ chatId, batchId }) {
    ...
    const retryable = status === 'SENDING'
  ...
  return {
    ...
    async claimIngestBatch({ chatId, batchId, fingerprint, requestIds, now = new Date(), expiresAt }) {
      ...
      if (existing.status === 'SENT') {
        return { outcome: 'COMPLETE', message_id: existing.messageId || '' };
      }
      const currentLease = Date.parse(String(existing.leaseExpiresAt || ''));
      if (Number.isFinite(currentLease) && currentLease > now.getTime()) {
        return { outcome: 'IN_FLIGHT', lease_expires_at: existing.leaseExpiresAt };
      }
      const resumed = await patchIngestContext({
        formMessageId: key.formMessageId,
        expectedLeaseExpiresAt: existing.leaseExpiresAt,
        context,
      });
      if (resumed) return { outcome: 'RESUMED', lease_expires_at: leaseExpiresAt };

      // A concurrent owner may have completed or renewed the lease after our read.
      // Re-read instead of overwriting that newer state.
      const latest = parseBatchContext(await getBatchRow(key.formMessageId));
      if (!latest || latest.kind !== 'dispatch_ingest') {
        throw new DispatchStoreError('INGEST_CLAIM_LOST', '发送幂等状态已失效', 409);
      }
      if (latest.fingerprint !== fingerprint) return { outcome: 'CONFLICT' };
      if (latest.status === 'SENT') {
        return { outcome: 'COMPLETE', message_id: latest.messageId || '' };
      }
      const latestLease = Date.parse(String(latest.leaseExpiresAt || ''));
      if (Number.isFinite(latestLease) && latestLease > now.getTime()) {
        return { outcome: 'IN_FLIGHT', lease_expires_at: latest.leaseExpiresAt };
      }
