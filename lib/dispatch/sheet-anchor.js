...
export function inspectLatestDailyAnchor(rows, {
  ...
  for (let index = rows.length - 1; index >= 0; index -= 1) {
    ...
    // The closest relevant row is authoritative. Never skip an invalid manual
    // assignee and revive an older anchor, otherwise chunked scans can dispatch
    // from stale sheet state.
    return {
      anchor: null, previousAssignee: assignee,
      hasRelevantAssignment: true, invalidAssignee: assignee,
    };
  }
  return {
