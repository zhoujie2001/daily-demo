...
export function inspectLatestDailyAnchor(rows, { targetDay, roster, projectValue = '' }) {
  const allowed = new Set((roster || []).map((name) => String(name).trim()));
  let hasRelevantAssignment = false;
  let invalidAssignee = null;
  for (let index = rows.length - 1; index >= 0; index -= 1) {
    ...
    hasRelevantAssignment = true;
    if (allowed.has(assignee)) {
      return { anchor: assignee, hasRelevantAssignment, invalidAssignee: null };
    }
    invalidAssignee ||= assignee;
  }
  return { anchor: null, hasRelevantAssignment, invalidAssignee };
}
