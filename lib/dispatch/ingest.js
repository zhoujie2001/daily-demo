...
function localPromoRejectReasonText(item) {
  return [
    ...
    item?.rejectReason,
    item?.['拒绝理由'],
    // Some upstream monitors only include the reject reason in the generated
    // BESS request name. Keep this as a fallback so already-normalized payloads
    // do not bypass the local-promo skip gate.
    item?.request_name,
    item?.requestName,
    item?.['需求名称'],
  ].map(normalizeRejectReasonText).filter(Boolean).join('\n');
}
