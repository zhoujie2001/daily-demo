...
begin
  ...
  -- 原子接单 RPC 必须存在、为 SECURITY DEFINER，并且仅 service_role 可执行。
  v_function := to_regprocedure(
  ...
  if v_function is null then
    ...
    if not exists (
      ...
      where p.oid = v_function and p.prosecdef
        and exists (
          select 1 from unnest(p.proconfig) as config(value)
          where split_part(config.value, '=', 1) = 'search_path'
            and btrim(translate(split_part(config.value, '=', 2), chr(34), '')) = ''
        )
    ) then
      v_errors := array_append(v_errors, 'bess_claim_ingest 必须为 SECURITY DEFINER 且 search_path 为空');
    end if;
    if exists (
      select 1 from pg_catalog.pg_proc p,
        lateral aclexplode(coalesce(p.proacl, acldefault('f', p.proowner))) acl
      where p.oid = v_function and acl.grantee = 0 and acl.privilege_type = 'EXECUTE'
    ) then v_errors := array_append(v_errors, 'PUBLIC 仍可执行 bess_claim_ingest'); end if;
    if exists (select 1 from pg_catalog.pg_roles where rolname = 'anon')
       and has_function_privilege('anon', v_function, 'EXECUTE')
    then v_errors := array_append(v_errors, 'anon 仍可执行 bess_claim_ingest'); end if;
    if exists (select 1 from pg_catalog.pg_roles where rolname = 'authenticated')
       and has_function_privilege('authenticated', v_function, 'EXECUTE')
    then v_errors := array_append(v_errors, 'authenticated 仍可执行 bess_claim_ingest'); end if;
    if exists (select 1 from pg_catalog.pg_roles where rolname = 'service_role')
       and not has_function_privilege('service_role', v_function, 'EXECUTE')
    then v_errors := array_append(v_errors, 'service_role 缺少 bess_claim_ingest EXECUTE'); end if;
  end if;
